import { AnyPgColumn, pgTable, timestamp, varchar, uuid, jsonb, numeric, index, uniqueIndex, primaryKey, check } from "drizzle-orm/pg-core";
...
export const listItems = pgTable("list_items", {
    ...
    index("user_list_item_title_index").on(table.userId, lower(table.title)),
    index("user_list_item_list_index").on(table.userId, table.listId),
]);
...
export const tags = pgTable("tags", {
    ...
    index("user_tag_name_index").on(table.userId, lower(table.name)),
]);
...
export const exchangeParticipants = pgTable("exchange_participants", {
    ...
    uniqueIndex("exchange_participant_unique").on(table.exchangeId, table.personId),
...
export const exchangeExclusions = pgTable("exchange_exclusions", {
    exchangeId: uuid("exchange_id").notNull().references(() => exchanges.id, { onDelete: "cascade" }),
    personId1: uuid("person_id_1").notNull().references(() => persons.id, { onDelete: "cascade" }),
    personId2: uuid("person_id_2").notNull().references(() => persons.id, { onDelete: "cascade" }),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow().$onUpdate(() => new Date()),
}, (table) => [
    primaryKey({ columns: [table.exchangeId, table.personId1, table.personId2] }),
    uniqueIndex("exchange_exclusion_unique").on(table.exchangeId, table.personId1, table.personId2),
...
export const exchangeAssignments = pgTable("exchange_assignments", {
    ...
    primaryKey({ columns: [table.exchangeId, table.giverId, table.receiverId] }),
    uniqueIndex("exchange_assignment_unique").on(table.exchangeId, table.giverId),
    uniqueIndex("exchange_receiver_unique").on(table.exchangeId, table.receiverId),
