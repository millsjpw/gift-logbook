...
export async function setup() {
  ...
  if (!process.env.DB_URL) {
    ...
    console.error('  export DB_URL_TEST="postgresql://postgres:postgres@localhost:5433/gift_logbook_test?sslmode=disable"');
  ...
  try {
    ...
    try {
      const info = await checkSql.unsafe("select current_database() as db, inet_server_port() as port");
    } catch (err) {
      console.warn('DB connectivity check failed (will continue):', err && (err as Error).message);
    } finally {
      await checkSql.end({ timeout: 0 });
    }
  } catch (err) {
    console.warn('Failed to perform DB connectivity check:', err && (err as Error).message);
  }
  ...
  async function truncateAll() {
    ...
    try {
      ...
      const tables = rows.map(r => r.tablename).filter(t => t !== 'drizzle_migrations');
      if (tables.length > 0) {
