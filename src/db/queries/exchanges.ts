...
export async function createExchange(userId: string, name: string) {
    ...
    const [createdExchange] = await db.insert(exchanges).values(exchange).returning();
...
export async function getExchangesByName(userId: string, name: string) {
    const exchangesByName = await db.select().from(exchanges).where(
        and(
            eq(exchanges.userId, userId),
            like(exchanges.name, `%${name}%`)
        )
    );
...
export async function updateExchange(id: string, name?: string) {
    ...
    const [updatedExchange] = await db.update(exchanges).set(updateData).where(eq(exchanges.id, id)).returning();
