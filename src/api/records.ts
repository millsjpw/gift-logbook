...
export async function handleAddRecord(req: Request, res: Response) {
    ...
    const record: GiftRecord = await recordsService.addRecord(userId, personId, itemText, amount, date ? new Date(date) : undefined, meta);
...
export async function handleUpdateRecord(req: Request, res: Response) {
    ...
    if (!itemText && !amount && !date && !meta) {
        throw new BadRequestError("At least one field (itemText, amount, date, meta) must be provided for update");
    }

    const updatedRecord = await recordsService.updateRecord(userId, recordId, itemText, amount, date ? new Date(date) : undefined, meta);
...
export async function handleDeleteRecordsByPersonId(req: Request, res: Response) {
    const userId = req.auth!.userId;
    const personId = req.params.personId as string;
...
export async function handleAddTagToRecord(req: Request, res: Response) {
    ...
    const updatedRecord = await recordsService.addTagToRecord(userId, recordId, tag);
    respondWithJSON(res, 200, updatedRecord);
}
...
export async function handleRemoveTagFromRecord(req: Request, res: Response) {
    ...
    const updatedRecord = await recordsService.removeTagFromRecord(userId, recordId, tag);
    respondWithJSON(res, 200, updatedRecord);
}
