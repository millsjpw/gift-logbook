...
export async function handleGetPeopleCreatedByUser(req: Request, res: Response) {
    const userId = req.auth?.userId;
    if (!userId) {
...
export async function handleUpdatePerson(req: Request, res: Response) {
    ...
    if (!name && !meta) {
        throw new BadRequestError("At least one field (name, meta) must be provided for update");
    }
    ...
    const updatedPerson = await personService.updatePerson(userId, personId, name, meta);
...
export async function handleDeletePeopleCreatedByUser(req: Request, res: Response) {
    const userId = req.auth?.userId;
    if (!userId) {
