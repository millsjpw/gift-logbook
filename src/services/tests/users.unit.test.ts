...
vi.mock('../auth.js', () => ({ hashPassword: vi.fn().mockResolvedValue('hashed') }));
...
describe('users service', () => {
  it('addUser hashes password and creates user', async () => {
    const fakeUser = { id: 'u1', name: 'A', email: 'a@b', hashedPassword: 'hashed' };
  ...
  it('updateUser hashes password when provided', async () => {
    ...
    expect(userDb.updateUser).toHaveBeenCalledWith('u1', 'B', undefined, 'hashed');
