...
describe('persons service', () => {
  it('updatePerson throws NotFoundError when person not found', async () => {
    ...
    await expect(personsService.updatePerson('u1', 'p1', 'Name')).rejects.toThrow(NotFoundError);
  });

  it('updatePerson throws UserForbiddenError when not owner', async () => {
    (personsDb.getPersonById as any).mockResolvedValue({ id: 'p1', userId: 'other' });
    await expect(personsService.updatePerson('u1', 'p1', 'Name')).rejects.toThrow(UserForbiddenError);
  });

  it('updatePerson calls db when owner', async () => {
    (personsDb.getPersonById as any).mockResolvedValue({ id: 'p1', userId: 'u1' });
    (personsDb.updatePerson as any).mockResolvedValue({ id: 'p1', name: 'New' });
