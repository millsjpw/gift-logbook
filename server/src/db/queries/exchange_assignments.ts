...
export async function addAssignmentToExchange(
  exchangeId: string,
  giverId: string,
  receiverId: string,
) {
...
export async function getNextRound(exchangeId: string) {
  const lastRound = await db
    ...
    .orderBy(desc(exchangeAssignments.createdAt))
    .limit(1);
...
export async function getAssignmentsByExchangeId(exchangeId: string) {
  const assignmentsList = await db
    ...
    .from(exchangeAssignments)
    .where(eq(exchangeAssignments.exchangeId, exchangeId))
    .innerJoin(person1, eq(exchangeAssignments.giverId, person1.id))
    .innerJoin(person2, eq(exchangeAssignments.receiverId, person2.id));
  return assignmentsList;
}

export async function getAssignmentForGiverInExchange(
  exchangeId: string,
  giverId: string,
) {
...
export async function removeAssignmentFromExchange(
  exchangeId: string,
  giverId: string,
) {
  await db
    .delete(exchangeAssignments)
    .where(
      and(
        eq(exchangeAssignments.exchangeId, exchangeId),
        eq(exchangeAssignments.giverId, giverId),
