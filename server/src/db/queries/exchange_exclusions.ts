...
export async function addExclusionToExchange(
  exchangeId: string,
  personId1: string,
  personId2: string,
) {
  ...
  const [createdExclusion] = await db
    .insert(exchangeExclusions)
    .values(exclusion)
    .returning();
  return createdExclusion;
}

export async function bulkInsertExclusions(
  exchangeId: string,
  ...
  const createdExclusions = await db
    .insert(exchangeExclusions)
    .values(exclusionRecords)
    .returning();
...
export async function getExclusionsForPersonInExchange(
  exchangeId: string,
  personId: string,
) {
  const exclusionsList = await db
...
export async function removeExclusionFromExchange(
  exchangeId: string,
  personId1: string,
  personId2: string,
) {
  await db
    .delete(exchangeExclusions)
    .where(
      and(
        eq(exchangeExclusions.exchangeId, exchangeId),
...
export async function removeExclusionsForPersonInExchange(
  exchangeId: string,
  personId: string,
) {
  await db
